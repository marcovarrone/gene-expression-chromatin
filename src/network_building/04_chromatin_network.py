...
def save_disconnected_nodes(hic, dataset, filename):
    ...
    os.makedirs('../../data/{}/disconnected_nodes'.format(dataset), exist_ok=True)
...
def multi_chromosome(args):
    ...
    if not os.path.exists(data_folder + filename + '.npy'):
        ...
        hics = [
            np.load(
                '../../data/{}/hic/{}_{}_{}_{}.npy'.format(args.dataset, args.type, i, i,
                                                                 args.resolution)) for i in range(1, 23)]
        ...
        if args.perc_inter is not None:
            ...
            hic_full = build_full_hic(args.dataset, args.type, args.resolution)
