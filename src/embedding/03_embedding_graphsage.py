...
parser.add_argument('-f', '--force-train', default=False, action='store_true')
...
from models.graphsage import GraphSAGELinkPredictor
...
adjacency = sps.load_npz(config['GRAPH'][args.dataset]).todense()
...
if args.embeddings:
    node_features = np.load('embeddings/' + str(args.dataset) + '/' + str(args.embeddings) + '.npy')
else:
    print("No embedding provided. Training featureless GraphSAGE.")
    node_features = np.ones((adjacency.shape[0], 50))
graphsage = GraphSAGELinkPredictor(graph=g_nx, node_features=node_features, p_test=args.p_test,
                                   learning_rate=args.learning_rate, save_model=args.save_model,
                                   embeddings=args.embeddings, dataset=args.dataset)
graphsage.fit(epochs=args.epochs, force_train=args.force_train)

if args.save_embedding:
    embeddings = graphsage.embeddings
    print('Saving embedding in embeddings/' + str(args.dataset) + '/' + str(graphsage) + '.npy')
    np.save(
