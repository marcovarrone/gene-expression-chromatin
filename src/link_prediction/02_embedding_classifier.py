...
def main(args):
    ...
    if args.coexp_features:
        ...
        experiment_id = '{}_{}_{}_{}_{}_{}_{}'.format(args.dataset, args.classifier, args.n_splits, coexp_thrs,
                                                      args.method, args.embedding, args.aggregator)
    else:
    ...
    if args.method == 'topological':
        ...
        if args.full_coexpression:
            filename = '{}chr_all/{}_{}_{}_{}_{}.pkl'.format('test/' if args.test else '', args.classifier, args.method,
                                                             args.embedding, args.aggregator, coexp_thrs)
        else:
            filename = '{}chr_{}/{}_{}_{}_{}_{}.pkl'.format('test/' if args.test else '', args.chr_src, args.classifier,
                                                            args.method, args.embedding, args.aggregator, coexp_thrs)
    ...
    if not os.path.exists('../../results/{}/{}'.format(args.dataset, filename)) or args.force:
        ...
        if args.full_coexpression:
            shapes = [np.load(
                '../../data/{}/coexp/coexpression_chr_{}_{}.npy'.format(args.dataset, i, i)).shape for i
                      in
                      range(1, 23)]
        ...
        if args.method == 'topological':
            ...
            X = method_embedding(args, n_nodes, start_src, end_src, edges, non_edges)
        ...
        if args.test:
            ...
            for i in range(args.n_iter):
                results_iter = evaluate_embedding(X_train, y_train, args.classifier, verbose=1, clf_params={'n_estimators': 100}, n_splits=args.n_splits, mask=mask)
