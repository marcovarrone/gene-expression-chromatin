...
if args.valid_size:
    ...
    np.save(str(data_folder)+'/GSE92743_X_genes' + str(representation), X_train)

    print('Saving validation set with size ' + str(X_valid.shape))
    np.save(str(data_folder)+'/GSE92743_y_genes' + str(representation), X_valid)
else:
    print('Saving train set with size ' + str(X.shape))
    np.save(str(data_folder)+'/GSE92743_X_genes'+str(representation), X)
