...
if __name__ == '__main__':
    ...
    interactions = np.load('data/{}/interactions/interactions_{}.npy'.format(args.dataset,args.interactions))
    ...
    for i in range(10):
        ...
        for train_index, val_index in skf.split(X_train, y_train):
            ...
            y_pred = np.zeros(y_train[val_index].shape)
            for i in range(X_train[val_index].shape[0]):
                edge = X_train[val_index][i]
                y_pred[i] = interactions[edge[0], edge[1]]
    ...
    with open('results/{}/chr_{:02d}/trivial.pkl'.format(args.dataset, args.chr), 'wb') as file_save:
