...
class Autoencoder(object):
    ...
    def __init__(self, in_dim, embedding_size, encoder_sizes=None, decoder_sizes=None, learning_rate=0.001,
                 ...
                 dropout_in=None, dropout=None, max_norm=None, initializer='glorot_uniform',
                 patience=10, checkpoint_every=0, save_model=False, run_folder=None, offset=0):
        super().__init__()
        self.in_dim = in_dim
    ...
    def _build_model(self):
        ...
        for i, encoder_size in enumerate(self.encoder_sizes):
            x = Dense(units=encoder_size, activation=self.activation, kernel_initializer=self.initializer)(x)
            if self.dropout:
                x = Dropout(self.dropout)(x)

        x = Dense(units=self.embedding_size, activation=self.activation, kernel_initializer=self.initializer)(x)
        if self.dropout:
            x = Dropout(self.dropout)(x)
        ...
        for decoder_size in self.decoder_sizes:
            x = Dense(units=decoder_size, activation=self.activation, kernel_initializer=self.initializer)(x)
            if self.dropout:
                x = Dropout(self.dropout)(x)
    ...
    def fit(self, x=None, batch_size=128, epochs=1, verbose=1, callbacks=None, validation_split=0.,
        ...
        if os.path.isfile('models/' + str(self) + '_weights.h5') and not force_train:
            print("This model has already been trained. Loading from file")
