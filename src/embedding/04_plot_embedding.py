...
parser.add_argument('-t', '--technique', choices=['pca', 'tsne', 'umap'], required=True)
...
if __name__ == '__main__':
    ...
    if args.n_clusters:
        landmarks = np.load(
            'landmarks/' + str(args.dataset) + '/' + str(args.embeddings) + '_k' + str(args.n_clusters) +
            '.npy')
    else:
    ...
    if args.technique == 'pca':
        ...
        technique = UMAP(n_neighbors=args.n_neighbors)
        name = 'umap_'+str(args.n_neighbors)

    if args.save_fig:
        filename = 'plots/' + str(args.dataset) + '/' + str(name) + '_' + str(args.embeddings) + '.png'
    print(landmarks)
